...
class RectangleTracker:
    def __init__(self, max_lost, max_dist, min_found=10):
...
class ImgSubscriber(Node):
    def __init__(self):
        ...
        self.curve_count = 0
        self.tracker = RectangleTracker(max_lost=20, max_dist=50, min_found=20) #신규 10프레임 실종 20프레임 / 50픽셀
        self.tilt_text = "" # 화면 출력
        self.curve_text = "" # 화면 출력
        self.recent_curve = deque(maxlen=30) # maxlen 프레임 중에서 최빈값
        self.recent_tilt = deque(maxlen=30)
        # 시간 디버깅
        self.frame_count = 0
        self.total_time = 0.0
    ...
    def color_image_callback(self, msg):
        ...
        # 2. HSV 흰색 범위 지정
        hsv_upper_white = np.array([180, 110, 255])
        ...
        for cnt in contours:     # 사각형 찾기
            ...
            if len(approx) == 4:  # 1. 꼭짓점이 4개면,
                ...
                if 3000 > area_rect > 500:  # 2. 외접 박스 면적이 500 이상, 3000이하면,
                    angles = []
        ...
        if len(candidates) >= 3:  # 1. 3개 이상일 때: 맨 위 점 제외하고 직선 근사              
            ...
            if tip_lost > 0:
                cv2.circle(bgr_image, (tip_x, tip_y), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
            else:
                cv2.circle(bgr_image, (tip_x, tip_y), 4, (0, 0, 255), -1) # 가장 위의 새로운 점은 빨강
            
            for acx, acy, ac_lost in candidates[:-1]:
                if ac_lost > 0:
                    cv2.circle(bgr_image, (acx, acy), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
                else:
                    cv2.circle(bgr_image, (acx, acy), 4, (255, 0, 0), -1) # 나머지 파랑
            ...
            if down_lost > 0:
                cv2.circle(bgr_image, (x1, y1), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
            else:
                cv2.circle(bgr_image, (x1, y1), 4, (0, 0, 255), -1) #  파랑
            if up_lost > 0:
                cv2.circle(bgr_image, (x2, y2), 3, (0, 255, 255), 2) # lost가  0이상이면 노랑
            else:
                cv2.circle(bgr_image, (x2, y2), 4, (255, 0, 0), -1) # 빨강
