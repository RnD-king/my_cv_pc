...
> 이렇게 3세 폴더를 싹 지우고 빌드하는 걸 클린빌드라고 부르며,  
> 빌드가 안 되거나 실행파일이 좀 이상하다 싶을 때 아몰라 클린빌드하면 해결되기도 한다)
...
단순하게 구성되어 있으니 추가로 원하는 기능이 있다면 마음껏 활용하여 본인만의 코드를 만들어보자.  
...
> 왼쪽 토픽에서 color_test_node를 클릭한 후, 파라미터 값을 바꿔보자  
> 간혹 Dynamic Reconfigure를 클릭해도 아무것도 뜨지 않을 때가 있는데, 그러면 뜰 때까지 계속 눌러주자

추가로, 출력 화면을 마우스 클릭하면 그 클릭한 부분의 HSV값을 터미널로 띄워주니 그걸 보고 해도 좋다.

\+ 다른 창에선 깊이 범위도 확인할 수 있어, 그냥 파라미터 튜닝할 때 안성맞춤인 코드.  
대회장에서 쏠쏠하게 써먹었다.
...
### 1) line_publisher.py
요약: 1. 화면 보정을 하고, 2. 점선을 탐지하고, 3. 점선 좌표들을 리스트로 모아 퍼블리시하는 코드.

```python
class RectangleTracker:
```
__\= 점선 추적기__  
카메라가 같은 점선을 가만히 보고 있더라도 분명 가끔은 1\~2프레임씩 인식을 못 하고 끊기는 일이 발생한다. 
(조건문의 문제이거나 카메라 노이즈, 흔들림 등의 이유로)  
혹은 같은 이유로, 점선이 없는 바닥을 보고 있더라도 1\~2프레임씩 노이즈를 점선으로 인식하는 일이 발생한다.  
...
또한, 로봇이 이동하면서 점선의 위치도 로봇 이동 방향과 반대로 카메라에서 이동하게 될텐데   
각 프레임마다 max_dist 이하만큼 움직인 점선만을 같은 점선이라 판단한다.  
...
노드를 실행하는 중에도 rqt를 통해 색 보정치나 점선 조건 등 여러 변수들을 실시간으로 변경할 수 있는 파라미터를 선언한다.  
self.declare_parameter("이름", 값)
파라미터는 앞으로 아주 유용하게 자주 쓰일 예정.
```python
 # 파라미터 적용
...
작년 대회에서 로봇은 가만히 서서 바닥을 15프레임(1초) 관찰하고, 그 관찰한 결과로부터 다음 동작을 결정했다.  
다음 동작이 끝나면 멈춰서 다시 바닥을 관찰하고\,\,\, 그런 식.

따라서 바닥을 관찰하며 탐지 코드(line_publisher, line_subscriber 등)를 실행하기 전에,  
영상처리 쪽에서 로봇의 모션이 모두 끝났다는 것을 알고 있어야 한다.
이를 위해 로봇의 동작이 끝나면, 모션을 담당하는 파트에서 motion_end 라는 이름의 토픽으로 True값을 한 번 보낸다.  
그 토픽을 line_subscriber.py가 받으면 이 모션 콜백 함수가 실행되어 탐지 코드를 시작할 수 있게 만들어 준다.
...
상태 변수의 의미:
- self.armed = False → 아직 모션 안 끝남 (motion_end 대기 중)
- self.armed = True → 모션 끝났고, 함수 실행 준비 완료 (motion_end 받음, 다음 line_callback에서 collecting=True로 전환)
- self.collecting = False → 데이터 수집 안 함 (15프레임 루프 미실행)
- self.collecting = True → 데이터 수집 중 (15프레임 동안 점선 분석)

따라서 line_callback 함수를 이 두 상태변수 관점에서 크게 본다면, 
```python
def line_callback():
...
평소에는 두 변수 모두 False 이므로, 모든 루프에서 
```python
    if not self.collecting:
        if not self.armed:
```
...
이로써 우리의 line_subscriber.py는 motion_end 토픽과, candidates 토픽 모두가 들어와야만 실행되는 구조를 갖추었다.

그렇다면 들 수 있는 의문:  
어차피 line_subscriber의 메인 실행 함수인 line_callback 함수는 LinePointsArray 메세지가 와야만 실행이 될텐데,  
방금과 같은 motion_end 감지 로직은 이곳이 아니라 line_publisher에 있어야 하는 것 아닐까?

답:  
맞는 말이다. 굳~이 line_subscriber에까지 이런 로직이 있을 필요는 없다.  

그럼에도 굳이굳이 subscriber에도 이 로직을 넣은 이유는,  
정말 만에 하나 LinePointsArray 메세지가 타이밍이 꼬여서 잘못 들어오는 불상사가 발생하더라도 이를 막아줄 수 있는 안전장치이기 때문.  
코딩에 안전장치는 성능을 해치지 않는 한 많을 수록 좋다!

참고로 현재 line_publisher에는 이 로직이 없지만, 실제라면 구현되어있어야 한다.  
지금 publisher 코드는 단순화된 버전이기 때문에 넣지 않았을 뿐이다.

아무튼 이제 본격적으로 line_callback 내용인 candidates로부터의 동작 판단 로직이 실행된다.  
(candidates: publisher에서 넘어온 중심점 좌표들)

동작 판단 기준을 대충 설명하자면,
1. 현재 코스의 기울기가 얼마인가 (self.tilt_text)  
    최상단 중심점을 제외한 중심점 좌표들을 1차식으로 근사한 후, 직선의 기울기를 구한다.  
    기울기가 클 수록 코스에 비뚤어진 채로 서있다고 판단  
2. 현재 코스가 직선구간인가 곡선구간인가 (self.curve_text)  
    최상단 중심점과 근사한 직선 사이의 거리를 구하고, 일정 거리 이상 떨어져있다면 현재 코스가 곡선 구간이라 판단.  
3. 점선들로부터 로봇이 얼마나 떨어져있는가 (self.out_text)  
    로봇의 중심점과 근사한 직선 사이의 거리를 구하고, 그 거리가 클 수록 코스로부터 로봇이 멀리 벗어나있다고 판단.
...
153줄부터 359줄까지 이런 내용이다... (경우의 수가 많아서 긴 것뿐 절대 어려운 로직이 들어간 건 아니다.  
그냥 단순 노가다. 여러분은 이런 거 하지마라...)  
ex) 기울기: straight / 직선or곡선: 왼쪽으로 곡선 구간 / out: 오른쪽으로 out  일 때 status는 몇이고..

그렇게 15프레임 동안 line_callback이 실행되며 15개의 동작 판단(status)이 모였을 때,  
그동안 보고 내린 동작 판단(status) 결정 중 가장 많이 나온 동작을 최종 선택(res)하여 퍼블리시한다.  
...
```
디버깅용 이미지 콜백함수.  
어차피 퍼블리셔 쪽에서 이미지 구독도 하고 화면 출력도 하기 때문에 여기에서 또 굳이 화면을 띄울 필요는 없지만,  
동작 판단 기준과 점선을 그리는 기준이 옳게 작동 중인지 화면 출력을 통해 눈으로 확인하기 위해 넣은 콜백함수.  
...
1. BallTracker의 부재  
    ...
    대신 추적은 있긴 해야하므로, 그 기능은 농구공 탐지문인
```python
for cnt in contours:
```
이 뒤에 조건문으로 구현되어있다  
...
### 1) hoop_detect
요약) 골대를 찾고 다음 동작을 퍼블리시

중심점 좌표를 퍼블리시지 않고 바로 동작 판단까지 한 코드 안에 끝내는 것과
골대를 찾는 조건 말고는 다를 게 크게 없다

### 2) hurdle_detect
요약) 허들을 찾고 다음 동작을 퍼블리시

골대와 같다

### 3) ball_and_hoop
요약) 공을 찾고 주운 뒤, 골대 탐지 모드 변경. 골대를 찾고 공을 던진 뒤, 다시 공 탐지 모드 변경.  
다음 동작을 퍼블리시하는 것 모두 이 코드 안에서 진행됨

멀리있는 공과 골대를 탐지할 땐 앞을 바라보는 카메라인 cam1,
공을 주울 땐 점선과 같이 바닥을 바라보는 카메라인 cam2를 사용하며, 상황에 맞게 모드를 천이한다.

때문에 이미지 콜백함수가 2개이다.

### 4) line_tracker
요약) CUDA로 전처리 > 점선 탐지 > 판단 퍼블리시 (미완성 파일!)
...
```
GPU 가속기인 CUDA를 사용한다는 것만 봐두면 좋을 것 같다.
